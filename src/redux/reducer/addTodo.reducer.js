const initState = {
  ...
  const setPersist = todos =>
    ...
    const setPersist3 = desc =>
    window.localStorage.setItem("todos3", JSON.stringify(desc));
  ...
  export const addTodo = (state = initState, action) => {
    switch (action.type) {
      case "PERSIST_TODOS":
        ...
        case "PERSIST_TODOS":
        const desc = JSON.parse(window.localStorage.getItem("todos3"));
        return { ...state, desc: todos ? todos : [] };
      case "ADD_TEXT":
